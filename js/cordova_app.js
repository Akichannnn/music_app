...
function createMusicControls(track) {
  ...
  MusicControls.create({
    ...
    startUpdatingElapsed(track_list);
...
function update_Music_Control(isPlaying) {
  if (!isApp || !window.MusicControls) return;
  MusicControls.updateIsPlaying(isPlaying,
    () => console.log("✅ updateIsPlaying:", isPlaying),
    err => console.error("❌ updateIsPlaying error:", err)
  );
}
...
function startUpdatingElapsed(track) {
  stopUpdatingElapsed(); // clear nếu đang chạy

  const mediaElement = play_audio_with_video ? video : audio;

  elapsedTimer = setInterval(() => {
    if (!mediaElement.paused && !mediaElement.ended) {
      MusicControls.updateElapsed(mediaElement.currentTime);
      // console.log("Update elapsed:", mediaElement.currentTime);
    }
  }, 1000); // update mỗi giây, đủ mượt
}
