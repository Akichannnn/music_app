...
const addPlayList_local = $library(".addPlayList_local"),
  ...
  ti_caret_down_btn = $(".ti-caret-down-btn"),
...
function save_list_playList(value) {
  ...
  let list_playList = JSON.parse(localStorage.getItem("list_playList")) || [];
  ...
  if (exists) {
    Swal.fire({
     title:"Bạn đã sử dụng tên này rồi!",
     icon: "fail",
     showConfirmButton: true,
     background:"black",
     confirmButtonText: "Được rồi"
...
function innerList_form_list_playList(name) {
 ...
 const listJson = localStorage.getItem(name);
  ...
  tracks.forEach(track => {
    ...
    ol.innerHTML = `
      <img class="img-kqtimkiem" src="${track.thumbnail || track.img || ''}" alt="Thumbnail">
...
const isNumber = /^\d+$/.test(String(return_what.id));
  if (isNumber) {
    ...
    playTrack(return_what);
    menu_nhac();
    innerInterface_Search(return_what);
    play_Track_Default = false;
    play_audio_with_video = false;
  }
    if(isApp){
